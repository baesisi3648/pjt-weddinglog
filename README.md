...
- **AI 폴백을 1급 시민으로** — 런타임 OpenAI 호출 (체크리스트 / 캡션 / 앨범 구성) 은 실패 / 키 없음 시 즉시 사전 정의 템플릿으로 전환. 모든 응답에 `source: "ai" | "template"` 을 함께 내려보내 UI 에서 *"자동 추천"* / *"기본 템플릿"* 배지로 출처를 노출. 키 없는 환경에서도 전체 UX 가 동일하게 동작.
- **AI 출력 검증** — JSON 파싱 실패 / 스키마 불일치 / 정통 양장본 톤을 위반하는 다중 사진 레이아웃 (T2~T4) 반환 시 거부 → 폴백.
- **결정 이력 보존** — 9개 기획 문서 + Council 리뷰 결과 (`docs/planning/`) 와 Lv3 파트너 계약 (`docs/partner-contract.md`) 으로 *왜 이렇게 만들었나* 의 근거를 코드 외부에 별도 보관.
