...
기존 본식 앨범은 *예쁜 사진을 모아 책으로 만드는 흐름* 입니다. WeddingLog 는 한 발 앞에 더 넣었습니다 — 책 첫 장에 **하객들의 손글씨 축하 메시지** + **QR 코드** 를 배치합니다. 양가 부모님이 첫 페이지를 펼쳤을 때 가장 먼저 보이는 것이 사진이 아니라, **결혼식에 함께해 준 모든 사람의 마음** 이 되도록.

차별화의 본질은 **"누가 만드냐"** 가 아니라 **"무엇을 책에 담을 것이냐"** 입니다.
...
### 6.7 더 시간이 있었다면 — 사업 비전 (구현 외)

이 과제의 안내문 가이드(*"콘텐츠가 본체, 책은 부가 기능"*) 를 *"부가 기능은 책 *하나만* 이 아니어도 된다"* 로 한 단계 더 확장한다면, 다음 세 가지가 자연스럽게 따라옵니다.

1. **식전영상 자동 생성** — Chapter 1(일상 데이트) ~ Chapter 2(본격 준비) 사진을 BGM과 함께 슬라이드쇼로 자동 구성. 본식 직전에 사용. 별도 영상 업체 컨택 / 시안 / 수정 사이클(2~3주) 을 0주로 줄이는 게 목표.
2. **모바일 청첩장** — 웨딩 촬영 사진(Chapter 3) 으로 청첩장을 자동 생성. 본식 D-60 시점에 발송 가능.
3. **포토카드 / 액자 인화** — 베스트 컷 몇 장을 낱장 인쇄 / 액자로 전환. 본식 직후 양가 인사 자리 선물용.

핵심 발상은 *콘텐츠 하나로 다양한 실물 상품으로 전환* 입니다. 캘린더에 일정·사진을 쌓는 그 콘텐츠 위에서 책 / 영상 / 청첩장 / 포토카드가 모두 파생되면, 커플당 객단가가 자연스럽게 올라가고 인쇄 API 회사 입장에서도 *책 1권 단위* 가 아니라 *책 + 부가 상품 묶음 단위* 로 거래가 커집니다. 이번 과제는 *부가 기능의 첫 번째* 인 책에 집중했고, 나머지는 다음 단계의 로드맵입니다.
