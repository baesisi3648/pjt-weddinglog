...
describe('CoupleProfileCard', () => {
  it('커플 이름이 렌더링된다', () => {
    ...
    render(<CoupleProfileCard couple={couple} album_order_deadline={{ days_remaining: 60, urgency: 'normal' }} />);
  ...
  it('주 D-day 카운트다운이 표시된다', () => {
    ...
    render(<CoupleProfileCard couple={couple} album_order_deadline={{ days_remaining: 30, urgency: 'normal' }} />);
  ...
  it('urgency=warning (D-21~D-14) → yellow-400 배지 표시', () => {
    ...
    render(<CoupleProfileCard couple={couple} album_order_deadline={{ days_remaining: 18, urgency: 'warning' }} />);
  ...
  it('urgency=urgent (D-14 이하, 미래) → red-500 배지 표시', () => {
    ...
    render(<CoupleProfileCard couple={couple} album_order_deadline={{ days_remaining: 10, urgency: 'urgent' }} />);
    const badge = screen.getByText(/앨범 주문 추천/);
    expect(badge).toBeInTheDocument();
  ...
  it('urgency=expired → "앨범 주문 기한 만료" gray 표시', () => {
    ...
    render(<CoupleProfileCard couple={couple} album_order_deadline={{ days_remaining: -5, urgency: 'expired' }} />);
  ...
  it('wedding_date가 과거 → "D+N" 형식으로 표시', () => {
    ...
    render(<CoupleProfileCard couple={couple} album_order_deadline={{ days_remaining: -10, urgency: 'expired' }} />);
