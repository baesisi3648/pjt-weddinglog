...
// @TEST src/components/CaptionSelector.test.jsx
...
/**
 * CaptionSelector — AI 캡션 추천 + 직접 입력 + 적용
 * @param {object} photo - { id, caption }
 * @param {object} event - { id, category, date, memo }
 * @param {Function} onApply - (caption, source) => void
 * @param {Function} onClose
 */
export default function CaptionSelector({ photo, event, onApply, onClose }) {
  const [loading, setLoading] = useState(false);
  const [suggestions, setSuggestions] = useState(null); // null | { captions, source }
  ...
  async function handleRequestAI() {
    ...
    try {
      ...
      setError(err.response?.data?.detail ?? 'AI 캡션 요청에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleApply() {
    ...
    try {
      ...
      onApply(caption, useCustom ? 'custom' : (suggestions?.source ?? 'ai'));
    } catch (err) {
      setError(err.response?.data?.detail ?? '캡션 저장에 실패했습니다.');
    } finally {
      setApplying(false);
    }
  }
