...
// 사용자가 직접 수정 가능. 시드 데이터 9개.
// 위치 정책: 좌측 4슬롯(top 5/27/49/71%) + 우측 4슬롯(top 5/27/49/71%) + 하단 가운데 1개.
// 각 메시지 maxWidth 220px 안에서 처리되므로 슬롯 간 22% 간격이면 겹치지 않음.
export const DEFAULT_BLESSINGS: BlessingMessage[] = [
  ...
  // 하단 가운데 (사진 인물 영역과 겹치지 않게 90% 위치)
  {
    id: 'b05',
    name: '신부 동생',
    relation: '처제',
    text: '형부 우리 언니 잘 부탁해요.',
    pos: { bottom: '4%', left: '50%' },
    rotate: -1,
    size: 'sm',
  },
];
