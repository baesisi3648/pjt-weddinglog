...
describe('Home', () => {
  ...
  it('throw 없이 마운트된다', () => {
    getHomeSummary.mockResolvedValue(MOCK_HOME);
    renderHome();
  });
  ...
  it('urgency=warning → yellow 배지 표시', async () => {
    ...
    getHomeSummary.mockResolvedValue(data);
    renderHome();
    await waitFor(() => {
  ...
  it('photo_counts.selected >= 1 → "앨범 주문하기" CTA 활성화', async () => {
    getHomeSummary.mockResolvedValue(MOCK_HOME); // selected: 5
    renderHome();
    await waitFor(() => {
  ...
  it('API 에러 시 에러 메시지 표시', async () => {
    getHomeSummary.mockRejectedValue(new Error('Network Error'));
    renderHome();
    await waitFor(() => {
