...
describe('EventDetail', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (getEvent as ReturnType<typeof vi.fn>).mockResolvedValue(mockEvent);
    (listPhotos as ReturnType<typeof vi.fn>).mockResolvedValue(mockPhotos);
  });
  ...
  it('로딩 중 스피너 표시', () => {
    (getEvent as ReturnType<typeof vi.fn>).mockReturnValue(new Promise(() => {})); // never resolves
    (listPhotos as ReturnType<typeof vi.fn>).mockReturnValue(new Promise(() => {}));
    renderEventDetail();
    expect(screen.getByRole('status')).toBeInTheDocument();
  });

  it('API 에러 시 에러 메시지 표시', async () => {
    (getEvent as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('네트워크 오류'));
    renderEventDetail();
    await waitFor(() => {
  ...
  it('삭제 버튼 클릭 → deletePhoto API 호출', async () => {
    (deletePhoto as ReturnType<typeof vi.fn>).mockResolvedValueOnce({});
    renderEventDetail();
  ...
  it('저장 버튼 클릭 → updateEvent API 호출', async () => {
    (updateEvent as ReturnType<typeof vi.fn>).mockResolvedValueOnce(mockEvent);
    renderEventDetail();
