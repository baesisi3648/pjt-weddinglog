...
describe('Timeline', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (toggleSelection as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 'p3', is_selected: true });
  });
  ...
  it('API 호출 중 로딩 상태를 표시한다', () => {
    ...
    (getTimeline as ReturnType<typeof vi.fn>).mockImplementation(() => new Promise((r) => { resolve = r; }));
    renderTimeline();
  ...
  it('API 에러 시 에러 메시지를 표시한다', async () => {
    (getTimeline as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('Network Error'));
    renderTimeline();
    await waitFor(() => {
